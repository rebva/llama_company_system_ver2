...
from langchain.schema import Document  # type hints
...
def build_vectorstore(*, docs: Optional[List[Document]] = None, persist: bool = True) -> Chroma:
    """
    ベクトルストアの構築を行う関数。
    :param docs: 事前に読み込んだドキュメント（省略時は内部でロード）
    :param persist: 永続化を行うかどうか
    :return: 登録済みの Chroma インスタンス
    """
    # 1) Document オブジェクトをすべて読み込む
    documents: List[Document] = docs or load_all_documents()
    ...
    # 明示的に永続化（from_documents でも保存されるが、persist=False 時の動作切り替え用）
    if persist:
        vectordb.persist()
